...
function (Ast.FunctionDef _ _ (Ast.GName _ name) args blocks) = "F Def " ++ name ++ "(" ++ unargs args ++ "):\n" ++ unblocks blocks
function (Ast.FunctionDec _ _ (Ast.GName _ name) args) = "F Dec" ++ name ++ "(" ++ unargs args ++ "):\n"
function _ = "Unknown\n"
...
unarg (Ast.ArgumentDef _ _ (Just (Ast.LName _ name))) = name
unarg (Ast.ArgumentDef _ _ (Just (Ast.GName _ name))) = name
...
unblock (Ast.BasicBlock _ name phis stmts flow) = "Block " ++ uname name ++ "\n" ++ unphis phis ++ unstmts stmts ++ unflow flow
...
brargs :: [Ast.Name Range] -> String
brargs [b] = uname b
brargs (b:x) = uname b ++ ", " ++ brargs x
brargs [] = ""
...
uncall (Ast.Call _ _ (Ast.GName _ name) args) = "  Call " ++ name ++ "(" ++ callargs args ++ ")"
uncall _ = "Unknown"
...
unconvop (Ast.ConvOpCall _ (Ast.ConvOp _ convop) _ value _) = "convop " ++ convop ++ " " ++ unvalue value
